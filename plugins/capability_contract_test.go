...
var phase25Skips = map[string]phase25Skip{
	// Empty after Phase 2.6 — every plugin × primitive × field cell now
	// passes the capability contract. The map is kept (rather than
	// removed entirely) so future SPEC additions can be staged here
	// while their plugin honoring is wired up, per the original
	// IMPLEMENTATION_PLAN.md §4.3 / §7.1 design.
}
